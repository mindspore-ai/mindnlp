...
def main(opt):
    ...
    with open(opt.pred_file) as f:
        preds = json.load(f)
    if opt.na_prob_file:
        with open(opt.na_prob_file) as f:
    ...
    exact_thresh = apply_no_ans_threshold(exact_raw, na_probs, qid_to_has_ans, opt.na_prob_thresh)
    f1_thresh = apply_no_ans_threshold(f1_raw, na_probs, qid_to_has_ans, opt.na_prob_thresh)
    ...
    if opt.na_prob_file and opt.out_image_dir:
        run_precision_recall_analysis(out_eval, exact_raw, f1_raw, na_probs, qid_to_has_ans, opt.out_image_dir)
        histogram_na_prob(na_probs, has_ans_qids, opt.out_image_dir, "hasAns")
        histogram_na_prob(na_probs, no_ans_qids, opt.out_image_dir, "noAns")
    if opt.out_file:
