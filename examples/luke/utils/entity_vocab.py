...
DUMPDB = None  # global variable used in multiprocessing workers
...
class EntityVocab:
    ...
    def _count_entities(title: str) -> Dict[str, int]:
        ...
        """
        counter = Counter()
        for paragraph in DUMPDB.get_paragraphs(title):
            for wiki_link in paragraph.wiki_links:
                title = DUMPDB.resolve_redirect(wiki_link.title)
                counter[title] += 1
        return counter
